...
def main(load_dir):
    ...
    if not input_dir.is_dir():
        ...
        for mp4_path in list(load_dir.glob('**/video.mp4')):
            date_time = mp4_path.parent.name
            out_path = input_dir.joinpath(f"{date_time}.mp4")
    ...
    if (not mapping_vid_dir.is_dir()):
        ...
        max_size = -1
        path = None
        for mp4_path in list(input_dir.glob('*.mp4')):
    ...
    for mp4_path in list(input_dir.glob('**/*.mp4')):
        ...
        out_video_path = this_out_dir.joinpath(mp4_path.name)  # <load_dir>/demos/<out_dname>/*.mp4
        ...
        imu_load_dir = load_dir.joinpath(mp4_path.name[0: -4])
        """
        Remarks: 
